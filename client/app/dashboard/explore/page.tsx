...
export default function ExplorePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 animate-fade-in pb-20">
      {/* Hero Header */}
      <div className="relative overflow-hidden rounded-[3rem] bg-slate-950 p-12 text-white shadow-2xl shadow-slate-900/20">
        ...
        <div className="relative z-10 grid lg:grid-cols-[1fr_auto] gap-12 items-center">
          <div className="space-y-6">
            ...
            <h1 className="text-5xl lg:text-7xl font-black uppercase tracking-tighter leading-[0.85]">
              Scam <br />
              <span className="text-emerald-500 italic">Registry</span>
            </h1>
            <p className="text-slate-400 text-lg max-w-xl font-medium leading-relaxed opacity-80">
              Synchronized intelligence on travel risks and safety protocols. Verified by AI and field operatives across 195 nations.
            </p>
          </div>

          <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-[2.5rem] w-full lg:w-[400px] shadow-inner">
             <form onSubmit={handleSearch} className="space-y-4">
                ...
                <button 
                  type="submit"
                  className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-4 rounded-2xl transition-all shadow-xl shadow-emerald-900/40 uppercase tracking-widest text-sm active:scale-95"
                >
                  Query Intelligence
                </button>
             </form>
      ...
      <div className="flex flex-wrap items-center justify-between gap-6 px-4">
        ...
        <div className="flex items-center gap-4">
          {selectedCountry && (
            <button 
              onClick={clearSearch}
              className="px-6 py-2.5 bg-red-50 text-red-600 border border-red-100 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-red-100 transition-all"
            >
              Clear Matrix Filter
            </button>
          )}
          <button className="px-6 py-2.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-emerald-100 transition-all">
             Report Interaction
          </button>
        </div>
      </div>
        ...
        <div className="space-y-8">
          <div className="px-2">
            <h2 className="text-2xl font-black text-slate-950 uppercase tracking-tighter">
              {selectedCountry ? `Intelligence for ${selectedCountry}` : 'Operational Risk Feed'}
            </h2>
