...
function VerifyContent() {
  ...
  return (
    <div className="text-center space-y-8">
      {status === 'loading' && (
        <div className="space-y-6 py-8">
          <div className="relative mx-auto w-20 h-20">
            ...
            <Loader2 className="absolute inset-0 m-auto w-10 h-10 text-emerald-600 animate-spin" />
          </div>
      ...
      {status === 'success' && (
        <div className="space-y-6 py-8">
          <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-3xl flex items-center justify-center mx-auto shadow-xl shadow-emerald-900/5 rotate-3">
            <ShieldCheck size={40} strokeWidth={2.5} />
          </div>
          <div className="space-y-2">
            <h2 className="text-2xl font-black text-slate-950 uppercase tracking-tighter text-emerald-600">Verified!</h2>
            <p className="text-slate-500 text-sm font-medium px-4">Your email has been confirmed. You are ready to explore the world.</p>
          </div>
          <Link 
            href="/auth/login" 
            className="inline-flex items-center gap-2 px-8 py-4 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200 group"
          >
            Go to Login
      ...
      {status === 'error' && (
        <div className="space-y-6 py-8">
          ...
          <div className="flex flex-col gap-3">
            <Link 
              href="/auth/signup" 
              className="text-sm font-bold text-emerald-600 hover:underline underline-offset-4 decoration-2"
            >
