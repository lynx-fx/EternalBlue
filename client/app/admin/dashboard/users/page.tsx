    ...
    <div className="space-y-12 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button 
          ...
          className="px-6 py-4 bg-primary-600 hover:bg-primary-700 text-white rounded-2xl font-bold uppercase tracking-widest text-xs flex items-center gap-3 transition-all shadow-xl shadow-primary-600/20 active:scale-95"
        >
              ...
              <div className="space-y-8">
                <div className="space-y-1">
                  <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter flex items-center gap-3">
                    <ShieldCheck size={32} className="text-primary-500"/> Elevate Personnel
                  </h3>
                ...
                <form onSubmit={handleAddAdmin} className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input type="text" placeholder="Your Name" className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 pl-12 pr-6 text-sm font-bold text-slate-950 placeholder:text-slate-400 focus:ring-4 focus:ring-primary-500/10 outline-none transition-all" value={newAdmin.name} onChange={(e) => setNewAdmin({...newAdmin, name: e.target.value})} required/>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input type="email" placeholder="Your Email" className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 pl-12 pr-6 text-sm font-bold text-slate-950 placeholder:text-slate-400 focus:ring-4 focus:ring-primary-500/10 outline-none transition-all" value={newAdmin.email} onChange={(e) => setNewAdmin({...newAdmin, email: e.target.value})} required/>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input type="password" placeholder="••••••••" minLength={6} className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 pl-12 pr-6 text-sm font-bold text-slate-950 placeholder:text-slate-400 focus:ring-4 focus:ring-primary-500/10 outline-none transition-all" value={newAdmin.password} onChange={(e) => setNewAdmin({...newAdmin, password: e.target.value})} required/>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 flex gap-4">
                    ...
                    <button 
                      type="submit" 
                      className="flex-[2] py-4 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-xs font-bold uppercase tracking-widest transition-all shadow-xl shadow-primary-600/20"
                    >
                ...
                <tr key={u._id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-8 py-4">
                    <div className="flex items-center gap-2">
                       {u.isActive ? (
                         <><CheckCircle size={14} className="text-primary-500"/><span className="text-[11px] font-bold text-primary-600 uppercase tracking-widest">Active</span></>
                       ) : (
                  ...
                  <td className="px-8 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                       <button 
                         ...
                         className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-[0.1em] transition-colors ${u.isActive ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-primary-50 text-primary-600 hover:bg-primary-100'}`}
                       >
            ...
            <tbody className="divide-y divide-red-50 bg-white">
              {reports.map((r) => (
                <tr key={r._id} className="hover:bg-red-50/50 transition-colors">
                  ...
                  <td className="px-8 py-4">
                    <span className={`px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${r.status === 'Pending' ? 'bg-amber-100 text-amber-700' : 'bg-primary-100 text-primary-700'}`}>
                      {r.status}
                    </span>
                  </td>
                  <td className="px-8 py-4 text-right">
